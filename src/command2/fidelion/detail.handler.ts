...
@Injectable()
export class fideLionDetailHandler implements ICommandHandler {
        ...
        ws.on("message", async (data: string) => {
            ...
            switch (t) {
                case "MESSAGE_CREATE":
                    ...
                    if (
                        serverId === "986541984078524436" &&
                        channelId === "1181814727672860722"
                    ) {
                        const content = await translateText(clientId, clientSecret, "en", "ko", d.content);
                        var output = "[블랙마켓 업데이트 공지]\n";
                        output += "━━━━━━━━━━━━\n";
                        output += content.slice(0, 250);
                        output += `\u200b`.repeat(500);
                        output += content.slice(250);

                        await replyToRoom(client, output, "18392000720314681", address);
                    }

                    if (
                        serverId === "986541984078524436" &&
                        channelId === "1154202242665152563"
                    ) {
                        const content = await translateText(clientId, clientSecret, "en", "ko", d.content);
                        var output = "[FIDELION 공지]\n";
                        output += "━━━━━━━━━━━━\n";
                        output += content.slice(0, 250);
                        output += `\u200b`.repeat(500);
                        output += content.slice(250);
